...
from ..dao.session_message_table import get_session_message, save_session_message, update_summary
...
def message_memory_optimize(
    ...
    try:
        ...
        if record is None:
            # 首次访问，创建新记录
            log.info("[Memory] New session detected, creating record")
        ...
        if uncompressed_count <= COMPRESSION_THRESHOLD:
            # 不需要压缩，构建返回消息
            log.info("[Memory] No compression needed")
            return _build_optimized_messages(current_summary, messages, current_index)

        # 4. 执行压缩
        log.info("[Memory] Compression triggered!")
        ...
        log.error("[Memory] Falling back to original messages")
