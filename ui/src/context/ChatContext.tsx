...
const initialState: ChatState = {
  messages: [],
  selectedNode: Object.keys(app?.canvas?.selected_nodes ?? {})?.length
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const [modelDownloadMap, setModelDownloadMap] = useState<
  ...
  const modelDownloadPolling = async () => {
    if (currentDownloadingId?.current?.length > 0) {
      const responses = await Promise.all(
        currentDownloadingId?.current?.map((id) => getProgress(id)),
      );
      ...
      responses?.forEach((response) => {
        if (!!response) {
          ...
          if (response.status === "completed" || response.status === "failed") {
            currentDownloadingId?.current?.splice(
              currentDownloadingId?.current?.indexOf(response.id),
              1,
            );
          }
        }
