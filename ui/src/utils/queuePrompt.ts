...
function updatePrompt(
  ...
  try {
    ...
    for (const config of paramConfigs) {
      ...
      prompt_output[config.nodeId]["inputs"][config.paramName] =
        config.paramValue;
    }
...
export async function getOutputImagesByPromptId(
  promptId: string,
        ...
        if (nodeOutput && nodeOutput.images) {
          ...
          for (const img of nodeOutput.images) {
            if (img && img.filename && img.type) {
              ...
              const urlBase =
                app?.graph._nodes_by_id?.[nodeId]?.imgs?.[0]?.currentSrc?.split(
                  "?",
                )[0];
              outputImages[nodeId].push(
                urlBase +
                  "?filename=" +
...
export function getOnlyOneImageNode() {
  ...
  if (saveNodeIds.length === 0 && previewNodeIds.length === 0) {
    throw new Error(
      "No SaveImage or PreviewImage node found, please add one to the graph",
    );
  }
...
export async function getOutputImageByPromptId(
  promptId: string,
  nodeId: Number,
): Promise<string | null> {
  ...
  if (
    outputImages[nodeId.toString()] &&
    outputImages[nodeId.toString()].length > 0
  ) {
