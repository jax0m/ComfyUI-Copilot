...
export function applyNewWorkflow(workflow: any): boolean {
  try {
    ...
    if (workflow.nodes) {
      console.log(
        "[graphUtils] Loading UI format workflow with nodes:",
      ...
      // api格式的工作流
      console.log(
        "[graphUtils] Loading API format workflow with node count:",
 ...
 * @returns 是否成功应用参数
 */
export function applyNodeParameters(
  nodeParams: Record<string, Record<string, any>>,
): boolean {
  try {
        ...
        for (const widget of node.widgets) {
          if (widget.name === paramName) {
            ...
            if (widget.value !== value) {
              ...
              hasChanges = true;
              console.log(
                `[graphUtils] Updated node ${nodeId}, parameter ${paramName} to:`,
                value,
              );
            }
            break;
    ...
    if (hasChanges) {
      app.graph.setDirtyCanvas(false, true);
      console.log(
        "[graphUtils] Applied node parameters and marked canvas as dirty",
      );
    }
...
export function applyParameterChanges(
  changes: Array<{ node_id: string; parameter: string; new_value: any }>,
): boolean {
  try {
