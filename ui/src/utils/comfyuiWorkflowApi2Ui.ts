...
function optimizeNodeLayout(workflow: any): void {
  ...
  console.log(
    `[Layout] Applied layer-based layout with ${maxLayer + 1} layers`,
  );
}
 ...
 * ```
 */
export async function apiToWorkflow(
  ...
  for (const [id, data] of Object.entries(apiData as any)) {
    ...
    const nodeTypeExists =
      nodeData.class_type in LiteGraph.registered_node_types;
  ...
  for (const [, data] of Object.entries(apiData as any)) {
    ...
    try {
      ...
      if (nodeInfo && nodeInfo[nodeData.class_type]) {
        ...
        console.log(
          `[API to Workflow] Processing ${nodeData.class_type} with input_order:`,
          inputOrder,
        );
        console.log(
          `[API to Workflow] Available widget inputs:`,
          Object.keys(widgetInputs),
        );
        ...
        for (const inputName of inputOrder) {
          ...
          const isConnectionInput =
            ...
            (inputType === "MODEL" ||
              ...
              inputType === "CLIP" ||
              inputType === "IMAGE" ||
              inputType === "CONTROL_NET" ||
              inputType === "STYLE_MODEL");
          ...
          if (!isConnectionInput) {
            ...
            if (widgetInputs.hasOwnProperty(inputName)) {
              ...
              console.log(
                `[API to Workflow] Added widget ${inputName}:`,
                value,
              );
              ...
              if (inputConfig[1] && inputConfig[1].control_after_generate) {
                ...
                console.log(
                  `[API to Workflow] Added control_after_generate for ${inputName}:`,
                  controlMode,
                );
              }
              ...
              console.log(
                `[API to Workflow] Added default widget ${inputName}:`,
                defaultValue,
              );
              ...
              if (inputConfig[1].control_after_generate) {
                ...
                node.widgets_values.push(controlMode);
                console.log(
                  `[API to Workflow] Added default control_after_generate for ${inputName}:`,
                  controlMode,
                );
              }
            }
        ...
        console.log(
          `[API to Workflow] Final ${nodeData.class_type} widgets_values:`,
          node.widgets_values,
        );
      } else {
      ...
      console.warn(
        `[API to Workflow] Failed to get node info for ${nodeData.class_type}:`,
        error,
      );
...
const simpleExample: ComfyWorkflowJSON = {
  ...
  nodes: [
    {
      ...
      pos: [687.7930297851562, -31.296039581298828],
      size: [315, 98],
      flags: {},
      order: 0,
      mode: 0,
    ...
    },
    {
      id: 2,
      type: "CLIPTextEncode",
      pos: [1114.2213134765625, -19.439258575439453],
    ...
    },
    {
      id: 3,
      type: "SaveImage",
      pos: [1891.88720703125, -318.9201354980469],
      size: [427.60394287109375, 432.4823303222656],
      flags: {},
      order: 4,
      mode: 0,
      inputs: [
    ...
    },
    {
      id: 4,
      type: "FluxGuidance",
      pos: [1531.7930908203125, -315.2960510253906],
      size: [315, 58],
      flags: {},
      order: 5,
      mode: 0,
    ...
    },
    {
      id: 5,
      type: "KSampler",
      pos: [1532.7786865234375, -191.07691955566406],
      size: [315, 262],
      flags: {},
      order: 7,
      mode: 0,
    ...
    },
    {
      id: 6,
      type: "VAEDecode",
      pos: [1641.7930908203125, 134.70396423339844],
      size: [210, 46],
      flags: {},
      order: 6,
      mode: 0,
    ...
    },
    {
      ...
      pos: [1126.6373291015625, 96.14139556884766],
      size: [315, 106],
      flags: {},
      order: 1,
      mode: 0,
    ...
    },
    {
      id: 8,
      type: "CLIPTextEncode",
      pos: [1075.120849609375, -315.2674865722656],
  ...
  links: [
    ...
    [3, 1, 1, 8, 0, "CLIP"],
    [4, 1, 1, 2, 0, "CLIP"],
    [5, 2, 0, 5, 2, "CONDITIONING"],
    [6, 5, 0, 6, 0, "LATENT"],
    [7, 1, 2, 6, 1, "VAE"],
    [8, 6, 0, 3, 0, "IMAGE"],
    [9, 4, 0, 5, 1, "CONDITIONING"],
    [10, 8, 0, 4, 0, "CONDITIONING"],
  ],
  groups: [],
  config: {},
  extra: {
    ...
    ds: {
      ...
      offset: [-330.427808521672, 638.4239195808423],
    },
...
(globalThis as any).testKSamplerParameterFix = async function () {
  ...
  try {
    ...
    const ksamplerNode = workflow.nodes.find(
      (node: any) => node.type === "KSampler",
    );
    if (!ksamplerNode) {
    ...
    console.log(
      "🔗 [Debug] Input connections:",
      ksamplerNode.inputs?.map((i: any) => i.name),
    );
    ...
    if (
      actualWidgets.length === expectedWidgets.length &&
      actualWidgets.every((val: any, i: number) => val === expectedWidgets[i])
    ) {
      console.log(
        "🎉 [Debug] SUCCESS! All KSampler parameters are correctly set and in proper order!",
      );
...
console.log(
  "🔧 [Debug] KSampler parameter fix loaded. Run 'testKSamplerParameterFix()' in console to test.",
);
