...
export async function getObjectInfoByClass(nodeClass: string): Promise<any> {
  try {
    const response = await fetch(`/object_info/${nodeClass}`, {
      method: "GET",
    });
    return await response.json();
  } catch (error) {
...
export async function manageQueue(options: {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to manage queue: ${response.status} ${response.statusText}`,
      );
    }
...
export async function interruptProcessing(): Promise<Response> {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to interrupt processing: ${response.status} ${response.statusText}`,
      );
    }
