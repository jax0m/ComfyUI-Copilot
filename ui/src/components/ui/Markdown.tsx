  ...
  return (
    <MemoizedReactMarkdown
      ...
      components={{
        ...
        table: ({ children }) => (
          <table className="border-solid border w-[100%] text-xs">
            {children}
          </table>
        ),
        th: ({ children }) => (
          <th className="border-solid border text-center pt-2 text-xs">
            {children}
          </th>
        ),
        td: ({ children }) => {
          ...
          return (
            <td className="border-solid border border-[#979797] text-center text-xs">
              {children}
            </td>
          );
        },
        code: ({ children }) => {
          ...
          return (
            <span className="relative group inline-flex items-center">
              <code className="text-xs bg-gray-100 text-gray-900 rounded px-1">
                {children}
              </code>
              <button
                onClick={handleCopy}
        ...
        pre: ({ children }) => {
          return (
            <pre className="text-xs bg-gray-100 text-gray-800 rounded p-2 overflow-x-auto">
              {children}
            </pre>
          );
        },
        img: ({ node, ...props }) => {
          ...
          if (srcStrs && srcStrs.length > 0) {
            isGif =
              srcStrs[0]?.endsWith(".gif") || srcStrs[0]?.endsWith(".webp");
          }
          return (
            <div className={`${isGif ? "" : "w-1/2"} mx-auto`}>
              <LazyLoadImage
                {...props}
                effect="opacity"
