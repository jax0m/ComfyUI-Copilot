...
const TITLE_EN =
  "Missing models detected. We recommend downloading the following models.";
...
const ModelOption: React.FC<IProps> = (props) => {
  ...
  const [selectedPathMap, setSelectedPathMap] = useState<
  ...
  useEffect(() => {
    ...
    modelList?.forEach((item) => {
      const index = modelPaths.findIndex(
        (path: string) => item.model_type === path,
      );
  ...
  const handleDownload = async (
    ...
    let body: Record<string, string | number> = {
      ...
      model_id: modelId,
      model_type:
        !!modelType && modelType !== "" ? modelType : selectedPathMap[id],
    };
  ...
  const renderDownloadProgress = (status: string) => {
    if (status === "completed") {
      return (
        <svg className="w-5 h-5" viewBox="0 0 1024 1024" fill="#13d320">
          <path d="M198.920606 353.438906l294.89452 398.030819-141.866143 105.106304-294.89452-398.030819 141.866143-105.106304Z"></path>
          <path d="M292.668567 679.383279l612.100893-449.37515 104.488343 142.3252-612.100893 449.37515-104.488343-142.3252Z"></path>
        </svg>
      );
  ...
  const columns: ColumnsType<DataType> = [
    {
      title: thMap.name,
      key: "modelId",
      render: (_, record) => <div>{`${record.Path}/${record.Name}`}</div>,
    },
    {
      title: thMap.dir,
      key: "directory",
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex flex-wrap gap-1">
          {record?.Libraries?.map((item: string) => (
            <Tag content={item} />
          ))}
        </div>
      ),
      ...
      render: (_, record) => (
        <div>
          {record.LastUpdatedTime
            ? new Date(record.LastUpdatedTime * 1000).toLocaleString()
            : ""}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="w-full h-full flex justify-center items-center">
          {!!modelDownloadMap[record.Id] &&
          modelDownloadMap[record.Id].status !== "failed" ? (
            renderDownloadProgress(modelDownloadMap[record.Id]?.status)
          ) : (
            <button
  ...
  return (
    <div className="mt-4 w-full h-auto flex-1 min-h-0 overflow-y-auto">
      {showTitle && (
