...
export function DebugResult({
  ...
  const renderContent = () => {
    ...
    try {
      ...
      if (response.ext) {
        ...
        let checkpointExt = response.ext.find(
          (item: any) =>
        ...
        if (
          checkpointExt &&
          checkpointExt.data &&
          checkpointExt.data.checkpoint_id
        ) {
          ...
          checkpointExt = response.ext.find(
          ...
          if (
            ...
            checkpointExt = response.ext.find(
              (item: any) => item.type === "debug_checkpoint",
            );
            if (
              checkpointExt &&
              checkpointExt.data &&
        ...
        const workflowUpdateExt = response.ext.find(
          (item: any) => item.type === "workflow_update",
        );
        ...
        modelSuggests =
          response.ext.find((item: any) => item.type === "param_update")?.data
            ?.model_suggest || [];
      }
    } catch (error) {
    ...
    const helpText = isWorkflowUpdate ? (
      <div className="mt-3 text-xs text-gray-700">
        💡 If you're not satisfied with the changes, click the restore button to
    ...
    return (
      <div
        ref={ref}
        className="sticky top-0 left-0 w-full bg-gray-100 p-4 rounded-lg overflow-hidden"
      >
          ...
          {!!checkPointId && (
            <div className="ml-2 flex-shrink-0">
              <RestoreCheckpoint
                ...
                onRestore={() => {
                  console.log(
                    `Workflow restored from ${isWorkflowUpdate ? "workflow update" : "debug"} checkpoint`,
                  );
                }}
                title={
                  isWorkflowUpdate
                    ? `Restore to this version (Version ${checkPointId})`
  ...
  return <BaseMessage name={name}>{renderContent()}</BaseMessage>;
}
