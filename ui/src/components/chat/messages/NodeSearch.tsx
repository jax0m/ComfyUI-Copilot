...
export function NodeSearch({
  ...
  const installedNodesList = nodes.filter((node: { name: string }) =>
    isNodeInstalled(node.name),
  );
  const uninstalledNodesList = nodes.filter(
    (node: { name: string }) => !isNodeInstalled(node.name),
  );
  ...
  return (
    <div className="rounded-lg bg-gray-50 p-3 text-gray-700 text-xs break-words overflow-visible">
      ...
      {uninstalledNodesList.length > 0 && (
        <div
          className={`space-y-3 ${installedNodesList.length > 0 ? "mt-4" : ""}`}
        >
      ...
      {hoveredNode && hoveredNodeData?.description && (
        <div
          className="fixed -translate-y-full z-[9999] w-64 p-2 
