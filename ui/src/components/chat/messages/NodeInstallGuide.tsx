...
export function NodeInstallGuide({
  content,
  onLoadSubgraph,
}: NodeInstallGuideProps) {
  const response = JSON.parse(content);
  const nodeInfos =
    response.ext?.find(
      (item: { type: string }) => item.type === "node_install_guide",
    )?.data || [];

  const uniqueNodeInfos = nodeInfos.reduce(
    (acc: any[], node: { name: any }) => {
  ...
  return (
    <div>
      <p>
        Before loading the graph to canvas, the following nodes need to be
        installed. Please visit the corresponding GitHub repositories to install
        them:
      </p>
      ...
      <p className="mt-4"></p>
      <p>
        After installation, please click the continue loading graph button to
        load the graph to the canvas:
      </p>
      <div className="mt-4">
        <button
