...
const NodeParamsCard = ({ content }: { content: React.ReactNode }) => {
  return (
    <div className="rounded-lg border border-green-200 bg-green-50 p-1 shadow-sm">
      ...
      <div className="markdown-content text-green-900">{content}</div>
    </div>
  );
};
...
export function AIMessage({
  ...
  const renderContent = () => {
    try {
      ...
      const guides =
        response.ext?.find((item) => item.type === "guides")?.data || [];

      // 检查是否有实时更新的ext数据
      const hasWorkflowUpdate = response.ext?.some(
        (item) => item.type === "workflow_update",
      );
      const hasParamUpdate = response.ext?.some(
        (item) => item.type === "param_update",
      );
      ...
      if (metadata?.intent) {
        ...
        if (intent === "node_explain") {
          return (
            <NodeExplainCard content={<Markdown response={response || {}} />} />
          );
        } else if (intent === "node_params") {
          return (
            <NodeParamsCard content={<Markdown response={response || {}} />} />
          );
        }
      ...
      return (
        <div className="space-y-3">
          {format === "markdown" && response.text ? (
            ...
            <p className="whitespace-pre-wrap text-left">{response.text}</p>
          ) : null}
              ...
              return (
                <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div className="text-xs text-green-600 mt-1">
                    Changes have been applied to the canvas. Debug process
                    continues...
                  </div>
                </div>
              );
