...
export const ChatInput = forwardRef<ChatInputRef, ChatInputProps>(
  (
    ...
    const updateModels = (
      list: { label: string; name: string; image_enable: boolean }[],
    ) => {
      const selectedModel = getLocalStorage(
        LocalStorageKeys.MODELS_POP_VIEW_SELECTED,
      );
      // 之前记录的能在列表中找到，使用之前的记录。否则使用列表第一项重置
      if (
        selectedModel &&
        list.findIndex((model) => model.name === selectedModel) !== -1
      ) {
      ...
      setLocalStorage(
        LocalStorageKeys.MODELS_POP_VIEW_LIST,
        JSON.stringify(list),
      );
      setModels(list);
    };
    ...
    useEffect(() => {
      ...
      setLocalStorage(
        LocalStorageKeys.MODELS_POP_VIEW_TIME,
      ...
      <div className={`relative ${uploadedImages.length > 0 ? "mt-12" : ""}`}>
        ...
        <div className="w-full flex flex-row justify-end p-1 gap-2">
          <ButtonWithModal
            ...
            renderModal={(onClose) => (
              <PackageDownloadModal onClose={onClose} />
            )}
          />
          <ButtonWithModal
            buttonClass="rounded-md bg-white border-none 
        ...
        {/* Bottom toolbar */}
        <div
          className="absolute bottom-2 left-3 right-12 flex items-center gap-2 
        ...
        >
          <div className="border-r border-gray-100 flex flex-row justify-center pr-2">
            ...
            >
              <RefreshCcw
                className={`w-4 h-4 transition-transform ${isLoadingModels ? "animate-spin" : ""}`}
              />
            </button>
