...
const LazyAIMessage = lazy(() =>
  import("./messages/AIMessage").then((m) => ({ default: m.AIMessage })),
);
const LazyUserMessage = lazy(() =>
  import("./messages/UserMessage").then((m) => ({ default: m.UserMessage })),
);
// Use lazy loading for components that are conditionally rendered
const LazyWorkflowOption = lazy(() =>
  import("./messages/WorkflowOption").then((m) => ({
    default: m.WorkflowOption,
  })),
);
const LazyNodeSearch = lazy(() =>
  import("./messages/NodeSearch").then((m) => ({ default: m.NodeSearch })),
);
const LazyDownstreamSubgraphs = lazy(() =>
  import("./messages/DownstreamSubgraphs").then((m) => ({
    default: m.DownstreamSubgraphs,
  })),
);
const LazyNodeInstallGuide = lazy(() =>
  import("./messages/NodeInstallGuide").then((m) => ({
    default: m.NodeInstallGuide,
  })),
);
const LazyDebugGuide = lazy(() =>
  import("./messages/DebugGuide").then((m) => ({ default: m.DebugGuide })),
);
const LazyDebugResult = lazy(() =>
  import("./messages/DebugResult").then((m) => ({ default: m.DebugResult })),
);
...
export function MessageList({
      ...
      if (
        ...
        try {
          ...
          if (response.ext) {
            // 添加调试日志
            console.log(
              `[MessageList] Processing AI message ${i} with ext:`,
              response.ext,
            );

            // 查找修改前的checkpoint信息（给用户消息）
            const checkpointExt = response.ext.find(
              (item: any) =>
                item.type === "workflow_rewrite_checkpoint" ||
                (item.type === "debug_checkpoint" &&
                  item.data?.checkpoint_type === "workflow_rewrite_start"),
            );

            // 查找修改后的版本信息（保留在AI响应中）
            const completeExt = response.ext.find(
              (item: any) => item.type === "workflow_rewrite_complete",
            );
            ...
            if (checkpointExt && i > 0) {
              ...
              console.log(
                `[MessageList] Previous message role:`,
                prevMessage.role,
              );
              ...
              if (prevMessage.role === "user") {
                ...
                console.log(
                  `[MessageList] Associated checkpoint to user message:`,
                  processedMessages[i - 1],
                );
              }
            }
            ...
            if (completeExt) {
              ...
              updatedResponse.ext = response.ext.filter(
                (item: any) =>
                  item.type !== "workflow_rewrite_checkpoint" &&
                  !(
                    item.type === "debug_checkpoint" &&
          ...
          // 忽略JSON解析错误
          console.warn(
            `[MessageList] Failed to parse AI message content:`,
            error,
          );
        }
      }
  ...
  useEffect(() => {
      ...
      requestAnimationFrame(() => {
        ...
        if (isLoadHistory.current) {
          ...
          if (
            ...
            scrollHeightLast.current -
              scrollTopLast.current -
              clientHeightLast.current <
              1
          ) {
    ...
    const handleScroll = () => {
      ...
      if (
        el.scrollHeight < scrollHeightLast.current &&
        scrollHeightLast.current > 0
      )
        return;
      if (isLoadHistory.current) {
  ...
  const renderMessage = (message: Message, index: number) => {
    ...
    if (message.role === "ai" || message.role === "tool") {
      ...
      try {
        ...
        // 获取扩展类型
        const workflowExt = response.ext?.find(
          (item: ExtItem) => item.type === "workflow",
        );
        const nodeExt = response.ext?.find(
          (item: ExtItem) => item.type === "node",
        );
        const downstreamSubgraphsExt = response.ext?.find(
          (item: ExtItem) => item.type === "downstream_subgraph_search",
        );
        const nodeInstallGuideExt = response.ext?.find(
          (item: ExtItem) => item.type === "node_install_guide",
        );
        const paramUpdateExt = response.ext?.find(
          (item: ExtItem) => item.type === "param_update",
        );
        const workflowUpdateExt = response.ext?.find(
          (item: ExtItem) => item.type === "workflow_update",
        );
        const debugCheckpointExt = response.ext?.find(
          (item: ExtItem) => item.type === "debug_checkpoint",
        );
        const workflowUpdateCompleteExt = response.ext?.find(
          (item: ExtItem) => item.type === "workflow_update_complete",
        );

        // 检查是否是工作流成功加载的消息
        const isWorkflowSuccessMessage =
          response.text ===
          "The workflow has been successfully loaded to the canvas";
          ...
          if (
            ...
            if (!processedUpdates.current.has(workflowUpdateKey)) {
              const applyWorkflowWithRetry = async (retryCount = 0) => {
                try {
                  const { applyNewWorkflow } =
                    await import("../../utils/graphUtils");
                  const success = applyNewWorkflow(workflow_data);

                  if (success) {
                    console.log(
                      "[MessageList] Successfully applied workflow update",
                    );
                    // 标记该更新已处理（只有成功时才标记）
                    processedUpdates.current.add(workflowUpdateKey);
                  } else {
                    console.warn(
                      `[MessageList] Failed to apply workflow update (attempt ${retryCount + 1})`,
                    );
                    // 重试最多3次
                    if (retryCount < 2) {
                      setTimeout(
                      ...
                      console.error(
                        "[MessageList] Workflow update failed after 3 attempts",
                      );
                    }
                  }
                } catch (error) {
                  console.error(
                    `[MessageList] Error in workflow update (attempt ${retryCount + 1}):`,
                    error,
                  );
                  // 重试最多3次
                  if (retryCount < 2) {
            ...
            if (!processedUpdates.current.has(paramUpdateKey)) {
              const applyParamsWithRetry = async (retryCount = 0) => {
                try {
                  ...
                  if (success) {
                    console.log(
                      `[MessageList] Successfully applied ${changesList.length} parameter changes`,
                    );
                    // 标记该更新已处理（只有成功时才标记）
                    processedUpdates.current.add(paramUpdateKey);
                  } else {
                    console.warn(
                      `[MessageList] Failed to apply parameter changes, changes is ${JSON.stringify(changesList)}, (attempt ${retryCount + 1})`,
                    );
                    // 重试最多3次
                    if (retryCount < 2) {
                      setTimeout(
                      ...
                      console.error(
                        "[MessageList] Parameter update failed after 3 attempts",
                      );
                    }
                  }
                } catch (error) {
                  console.error(
                    `[MessageList] Error in parameter update (attempt ${retryCount + 1}):`,
                    error,
                  );
                  // 重试最多3次
                  if (retryCount < 2) {
        ...
        if (response.ext && message.finished) {
          const checkpointExt = response.ext.find(
            (item: ExtItem) =>
              item.data &&
              item.data.checkpoint_id &&
              (item.type === "debug_checkpoint" ||
                item.type === "workflow_rewrite_complete" ||
                item.type === "workflow_update_complete"),
          );
            ...
            if (!processedUpdates.current.has(saveWorkflowUIKey)) {
              const saveWorkflowUIWithRetry = async (retryCount = 0) => {
                try {
                  ...
                  if (typeof window !== "undefined" && (window as any).app) {
                    ...
                    if (app.graph) {
                      ...
                      if (workflowUI) {
                        ...
                        if (result.success) {
                          console.log(
                            `[MessageList] Successfully saved workflow UI for checkpoint ${checkpointId}`,
                          );
                          processedUpdates.current.add(saveWorkflowUIKey);
                        } else {
                          console.warn(
                            `[MessageList] Failed to save workflow UI for checkpoint ${checkpointId}: ${result.message}`,
                          );
                          // 重试最多3次
                          if (retryCount < 2) {
                            setTimeout(
                            ...
                            console.error(
                              `[MessageList] Save workflow UI failed after 3 attempts for checkpoint ${checkpointId}`,
                            );
                          }
                        }
                      }
                  ...
                  console.error(
                    `[MessageList] Error saving workflow UI (attempt ${retryCount + 1}):`,
                    error,
                  );
                  // 重试最多3次
                  if (retryCount < 2) {
        ...
        if (workflowExt) {
          ExtComponent = (
            <Suspense
              key={`workflow_option_${message.id}`}
          ...
          const dsExtComponent = (
            <Suspense
              key={`down_stream_subgraph_${message.id}`}
              fallback={<div>Loading...</div>}
            >
          ...
          ExtComponent = (
            <Suspense
              key={`node_install_guide_${message.id}`}
              fallback={<div>Loading...</div>}
            >
                ...
                onLoadSubgraph={() => {
                  if (message.metadata?.pendingSubgraph) {
                    ...
                    if (selectedNode) {
                      ...
                      try {
                        for (const node of nodes as NodeWithPosition[]) {
                          if (node.id !== entryNodeId) {
                            ...
                            nodeMap[node.id] = addNodeOnGraph(node.type, {
                              pos: nodePosNew,
                            });
                          }
                        }
                      ...
                      alert(
                        "Please select a upstream node first before adding a subgraph.",
                      );
                    }
                    ...
                    if (workflow.nodes) {
                      ...
                      for (const node_id of node_ids) {
                        ...
                        if (node) {
                          ...
                          // 根据参数计算节点的高度
                          const inputCount = node.inputs
                            ? node.inputs.length
                            : 0;
                          const outputCount = node.outputs
                            ? node.outputs.length
                            : 0;
                          const widgetCount = node.widgets
                            ? node.widgets.length
                            : 0;
                          const param_count =
                            Math.max(inputCount, outputCount) + widgetCount;
          ...
          isWorkflowSuccessMessage ||
          message.format === "debug_guide"
        ) {
          // 使用DebugGuide组件来处理工作流成功加载的消息或debug_guide格式的消息
          ExtComponent = (
            <Suspense
              key={`debug_guide_${message.id}`}
              fallback={<div>Loading...</div>}
            >
              <LazyDebugGuide
                content={message.content}
        ...
        if (
          (isWorkflowSuccessMessage || message.format === "debug_guide") &&
          ExtComponent
        ) {
          return ExtComponent;
        }
        ...
        if (
          (workflowUpdateExt || paramUpdateExt) &&
          message.finished &&
          ExtComponent
        ) {
          return ExtComponent;
  ...
  useEffect(() => {
    ...
    if (
      ...
      for (let i = processedMessages?.length - 1; i >= 0; i--) {
        ...
        if (
          processedMessages[i].role === "user" ||
          processedMessages[i].role === "showcase" ||
          processedMessages[i].format === "debug_guide"
        ) {
          count++;
      ...
      setCurrentMessages([
        ...processedMessages.slice(endIndex, processedMessages?.length),
      ]);
      lastMessagesCount.current = processedMessages.length;
    }
  ...
  return (
    ...
    >
      {showLoadMoreButton.current && (
        <div key={"loadmore"} className="flex justify-center items-center">
          <button
      ...
      {currentMessages?.map(
        (message, index) =>
          !!message && renderMessage(message as Message, index),
      )}
