...
import {
  createRewriteExpert,
  deleteRewriteExpert,
  getRewriteExpertsList,
  updateRewriteExpert,
} from "../../apis/rewriteExpertApi";
...
const RewriteExpertModal: React.FC<IProps> = ({ onClose }) => {
  ...
  const handleSearchRewriteExpert = (
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      setModalContent(
        isAdd.current ? `Add error ${error}` : `Edit error ${error}`,
      );
    }
  };
      ...
      <div className="relative bg-white rounded-xl p-6 w-1/2 h-2/3 flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl text-gray-900 dark:text-white font-semibold">
            Set expert experience to help you rewrite workflow
          </h2>
          <button
            onClick={onClose}
            disabled={loading}
        ...
        {editData && (
          <div className="absolute top-0 left-0 w-full h-full bg-white rounded-xl p-6 z-5 overflow-y-auto">
