...
export const handleStartGeneration = async (
  ...
  generateParameterCombinations: (paramTestValues: {
  ...
  if (totalCombinations > 100) {
    setErrorMessage(
      `Cannot generate more than 100 images at once (currently: ${totalCombinations} images)`,
    );
    return;
  }
  ...
  if (paramCombinations.length === 0) {
    setErrorMessage(
      "No valid parameter combinations found. Please check your parameter selections.",
    );
  ...
  Object.keys(paramTestValues).forEach((nodeId) => {
    ...
    if (node) {
      ...
      selectedNodeInfoMap[nodeId] =
        node.type || node.comfyClass || "Unknown Node";
    }
  });
  ...
  try {
    ...
    const primaryNodeName =
      selectedNodeInfoMap[primaryNodeId] || "Unknown Node";
...
export const pollForImages = async (
  ...
  for (let i = 0; i < prompt_ids.length; i++) {
    ...
    try {
      ...
      const imageUrl = await getOutputImageByPromptId(
        promptId,
        Number(showNodeId),
      );
