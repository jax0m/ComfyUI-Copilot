...
export const resetAllStates = (
  ...
  setSelectedParams: React.Dispatch<
  ...
  setParamTestValues: React.Dispatch<
    React.SetStateAction<{ [nodeId: string]: { [paramName: string]: any[] } }>
  >,
  ...
  setModalImageParams: React.Dispatch<
    React.SetStateAction<{ [key: string]: any } | null>
  >,
  setTextInputs: React.Dispatch<
    React.SetStateAction<{ [nodeId_paramName: string]: string[] }>
  >,
...
export const handleApplySelected = async (
  ...
  if (selectedImageIndex !== null) {
    console.log(
      `Applied image ${selectedImageIndex + 1} with parameters:`,
          ...
          for (const paramName in paramTestValues[nodeId]) {
            ...
            if (paramOptions) {
              ...
              if (Array.isArray(paramOptions)) {
                optionsCount =
                  paramOptions.length > 0 ? paramOptions.length : 1;
              } else if (
                typeof paramOptions === "object" &&
                paramOptions !== null
              ) {
                // Handle both numeric and string keys in objects
                optionsCount =
                  Object.keys(paramOptions).length > 0
                    ? Object.keys(paramOptions).length
                    : 1;
              }
    ...
    if (selectedParams.nodeParams) {
      ...
      if (success) {
        console.log(
          "[stateManagementUtils] Successfully applied selected parameters to canvas",
        );
        ...
        console.warn(
          "[stateManagementUtils] Failed to apply selected parameters to canvas",
        );
      }
    }
  }
