...
} from "./screens/ConfirmConfigurationScreen";
...
import { HistoryItem, loadHistoryItems } from "./utils/historyUtils";
...
export const ParameterDebugInterface: React.FC<
  ParameterDebugInterfaceProps
> = ({ selectedNodes, visible, onClose }) => {
  ...
  const [paramTestValues, setParamTestValues] = useState<{
    [nodeId: string]: { [paramName: string]: any[] };
  }>({});
  ...
  const [inputValues, setInputValues] = useState<{
    [nodeId_paramName: string]: { min?: string; max?: string; step?: string };
  }>({});
  ...
  const [modalImageParams, setModalImageParams] = useState<{
    [key: string]: any;
  } | null>(null);

  // Add state for text inputs and AI writing modal
  const [textInputs, setTextInputs] = useState<{
    [nodeId_paramName: string]: string[];
  }>({});
  ...
  const [aiSelectedTexts, setAiSelectedTexts] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const handleTextInputChange = (
    ...
    const currentTexts = utilsHandleTextInputChange(
      nodeId,
      paramName,
      index,
      value,
  ...
  const handleAddTextInput = (nodeId: string, paramName: string) => {
    const updatedTexts = utilsHandleAddTextInput(
      nodeId,
      paramName,
      textInputs,
      updateState,
  ...
  const handleRemoveTextInput = (
    ...
    const updatedTexts = utilsHandleRemoveTextInput(
      nodeId,
      paramName,
      index,
      textInputs,
  ...
  const doSaveState = (key: StateKey, value: any) => {
    ...
    const nextForKey =
      isObj(prev) && isObj(value) ? { ...prev, ...value } : value;
    saveStateToLocalStorage({
      ...stateToSave,
      [key]: nextForKey,
    });
  ...
  useEffect(() => {
    if (!isGetLocalstorage || !selectedNodes || selectedNodes.length === 0)
      return;
    ...
    const selectedNodeIds = selectedNodes.map((node: any) =>
      node.id.toString(),
    );
    ...
    if (
      JSON.stringify(updatedParamTestValues) !== JSON.stringify(paramTestValues)
    ) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      const savedState = JSON.parse(
        localStorage.getItem(PARAM_DEBUG_STORAGE_KEY) || "{}",
      );
      ...
      try {
        ...
        if (savedState) {
          ...
          if (!selectedNodes || selectedNodes.length === 0) {
            console.log(
              "Not restoring parameter debug state: no nodes selected",
            );
            setIsLoading(false);
            return;
          }
          ...
          if (parsedState.currentScreen !== undefined) {
            ...
            setCurrentScreen(
              isEqual(selectedNodes, currentSelectedNodes.current)
                ? parsedState.currentScreen
                : 0,
            );
          }
          if (parsedState.selectedParams)
            setSelectedParams(parsedState.selectedParams);
          ...
          const hasImages =
            parsedState.generatedImages &&
            parsedState.generatedImages.length > 0;
          ...
          if (parsedState.completedCount !== undefined)
            setCompletedCount(parsedState.completedCount);
          if (parsedState.totalCount !== undefined)
          ...
          if (parsedState.generatedImages)
            setGeneratedImages(parsedState.generatedImages);
          if (parsedState.paramTestValues)
            setParamTestValues(parsedState.paramTestValues);
          ...
          if (parsedState.currentPage !== undefined)
  ...
  const updateParamTestValues = (
    nodeId: string,
    paramName: string,
    values: any[],
  ) => {
    utilsUpdateParamTestValues(
      nodeId,
      paramName,
      values,
      paramTestValues,
  ...
  const handleTestValueSelect = (
    nodeId: string,
    paramName: string,
    value: any,
    event?: React.MouseEvent,
  ) => {
    utilsHandleTestValueSelect(
      nodeId,
      paramName,
      value,
      paramTestValues,
  ...
  const closeImageModal = (event?: React.MouseEvent) => {
    utilsCloseImageModal(setModalVisible, event);
  };
          ...
          <div className="space-y-4">
            <div className="text-left mb-4">
              <h3 className="text-sm font-medium text-gray-800 mb-3">
                Batch-test parameters & find the best combo in one click.
              </h3>
            ...
            <div className="text-left">
              ...
              <div className="space-y-2 mb-4">
                <p className="text-sm text-gray-700">
                  <span className="font-bold">1 - </span>Click any node(hold
                  Shift to select multiple nodes)
                </p>
                <p className="text-sm text-gray-700">
                  <span className="font-bold">2 - </span>Set parameter ranges
                  and start batch generation.
                </p>
              </div>
