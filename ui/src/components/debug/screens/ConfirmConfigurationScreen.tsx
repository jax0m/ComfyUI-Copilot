...
export const ConfirmConfigurationScreen: React.FC<
  ConfirmConfigurationScreenProps
> = ({
  selectedNodes,
  paramTestValues,
  selectedParams,
  ...
  const [selectedImageNodeId, setSelectedImageNodeId] = useState<number | null>(
    null,
  );
  const [showNodeSelectionWarning, setShowNodeSelectionWarning] =
    useState(false);
  ...
  useEffect(() => {
    if (selectedNodes?.length > 0) {
      const node = selectedNodes?.find(
        (node) => node.type === "SaveImage" || node.type === "PreviewImage",
      );
      if (!!node) {
      ...
      <div className="mb-4 border-b pb-2 flex justify-between items-center">
        <div>
          <h3 className="text-base font-medium text-gray-800">
            Conform combinations
          </h3>
          <p className="text-xs text-gray-500">
            In the selected parameter combinations, there are {totalCount} total
            runs. Each run will generate a separate image.
          </p>
        </div>
        <button
