...
export default function WorkflowChat({
  ...
  const [dispatchEventType, setDispatchEventType] = useState<DispatchEventType>(
    DispatchEventType.NONE,
  );
  ...
  useEffect(() => {
    ...
    if (savedScreenState) {
      dispatch({
        ...
        payload: JSON.parse(savedScreenState),
      });
    }
    ...
    window.addEventListener(
      COPILOT_EVENTS.TOOLBOX_PARAMETERS,
      onToolBoxParameters,
    );
    window.addEventListener(
      COPILOT_EVENTS.TOOLBOX_DOWNSTREAMNODES,
      onToolBoxDownstreamNodes,
    );
  ...
  // 使用防抖处理宽度调整
  const handleMouseMoveForResize = React.useCallback(
    (e: MouseEvent) => {
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener(
        "mousemove",
        debouncedHandleMouseMoveForResize,
      );
  ...
  const onUsage = () => {
    handleSendMessageWithContent(
      `Reply in ${navigator.language} language: How does the ${selectedNode?.[0]?.type} node work? I need its official usage guide.`,
    );
  };

  const onParameters = () => {
    handleSendMessageWithContent(
      `Reply in ${navigator.language} language: Show me the technical specifications for the ${selectedNode?.[0].type} node's inputs and outputs.`,
    );
  };
  ...
  const onDownstreamNodes = () => {
    handleSendMessageWithIntent(
      "downstream_subgraph_search",
      getDownstreamSubgraphExt(),
    );
  };
  ...
  return (
    ...
    >
      <div className="absolute left-0 top-0 bottom-0 w-1 cursor-ew-resize hover:bg-gray-300" />
